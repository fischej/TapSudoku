//
//  Color-Cells.swift
//  TapSudoku
//
//  Created by Jeff Fischer on 12/13/22.
//

import SwiftUI

extension Color {
    static let squareStandard = Color(red: 0.22, green: 0.25, blue: 0.3)
    static let squareSelected = Color.blue
    static let squareSelectedHints = Color.indigo
    static let squareHighlighted = Color(red: 0.1, green: 0.15, blue: 0.2)

    static let squareTextCorrect = Color.white
    static let squareTextWrong = Color.red
    static let squareTextSame = Color.white
    static let squareTextGiven = Color.gray
    static let squareTextCaution = Color.yellow
}
